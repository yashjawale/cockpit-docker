 ...
 *
 * Dialog committing a container to a new image.
 */
...
import { quote_cmdline, unquote_cmdline } from '../lib/util.ts';

import type { Connection } from '../lib/rest.ts';
import type { DockerContainer, DockerImage } from '../lib/types.ts';
...
const ContainerCommitModal = ({ con, container, localImages }: {
    ...
    const [command, setCommand] = useState(quote_cmdline(container.Config?.Cmd));
    ...
    const handleCommit = (force: boolean) => {
        ...
        const full_name = `${imageName}:${tag !== "" ? tag : "latest"}`;
        ...
        const commitData: Record<string, string | boolean | string[]> = {};
        ...
        if (command.trim() !== "") {
            let cmdData = "";
            const words = unquote_cmdline(command.trim());
        ...
        client.commitContainer(con, commitData)
                .then(() => Dialogs.close())
                .catch(ex => {
                    setDialogError(cockpit.format(_("Failed to commit container $0"), container.Name)); // not-covered: OS error
